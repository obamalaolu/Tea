...
On initial scan we find port 3389 and 3000 opened on the second box.

Basic Web enumeration we see the box is running gittea version `Version: 1.21.2`
Checking the internet for known exploit but we cannot find any.


We find API under `$IP:3000/api/swagger#/`
There doesnt seam to be much information when we enumerate the application. 
...
Once we login we see some information about the Admin user `email for admin - gitea@tea.vl`

After further enumeration we find Runners with a label called `windows-latest`

follwing the quick guide `https://docs.gitea.com/usage/actions/quickstart` we can run commands on the windows machine.
...
We need to create a base64 powershell reverseshell.

one way is to use CyberChef `gchq.github.io` in recipe add `Encode Text` and `To Base64`
...
replace $IP & $PORT with your IP and prefered port, I used 443
...
view hidden files/folders in windows directory
`cmd /c dir /A`
...
Get Updog inorder to get upload/download to kali.
...
the _install folder has PSExec64 this can be used for Privilege Escalation. using SharpWSUS we can push out updates to 
...
You should retry this command below several times until user is part of the admin group.
```
SharpWSUS.exe approve /updateid:363a6caa-7624-459d-b912-0740166f92b1 /computername:dc.tea.vl /groupname:"Custome01"

```
You can login using the new user you have added to the admin group.
